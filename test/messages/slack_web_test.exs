defmodule Messages.SlackWebTest do
  ...
  test "push" do
    {:ok, response} = Jason.encode(%{ok: true,
                                     channel: "C1H9RESGL",
                                     ts: "1503435956.000247",
                                     message: %{ text: "Here's a message for you",
                                                 username: "ecto1",
                                                 bot_id: "B19LU7CSY",
                                                 type: "message",
                                                 subtype: "bot_message",
  ...
  test "delete" do
    http_response = %HTTPoison.Response{ body: "{\"ok\": true,\"channel\": \"C024BE91L\",\"ts\": \"1401383885.000061\"}"}
    expect(HTTPoison.BaseMock, :post, fn (_url, _payload, _headers) -> {:ok, http_response} end)
    {:ok, message} = Messages.Message.create(%{subject: "subject", body: "body"})
    {:ok, slack_message} = Messages.SlackMessage.create(%{slack_timestamp: "1503435956.000247",
